...
def extract_so_page(word):
    so_result = requests.get(f"{url}{word}")
...
def extract_so_job(last_page,word):
    jobs = []
...
def get_so_jobs(word):
    max_so_page = extract_so_page(word)
    jobs = extract_so_job(max_so_page,word)
    return jobs
